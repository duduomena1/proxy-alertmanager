...
Sistema de monitoramento e alertas que integra Grafana, Prometheus e Portainer para enviar notificações estruturadas ao Discord.

## ✨ Características Principais

- 🔍 **Monitoramento Ativo via Portainer**: Detecta containers DOWN/UP em tempo real (30s)
- 📊 **Alertas do Grafana**: CPU, Memória, Disco com formatação personalizada
- 🛡️ **Supressão Inteligente**: Evita spam de alertas repetidos com persistência de estado
- 🔄 **Blue/Green Deployment**: Supressão automática quando sibling está ativo
- 💾 **Persistência de Estado**: Mantém supressões entre restarts do container
- 🎨 **Formatação Rica**: Mensagens Discord com embeds, cores e informações detalhadas
